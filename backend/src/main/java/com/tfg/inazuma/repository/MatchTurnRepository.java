...
public interface MatchTurnRepository extends JpaRepository<MatchTurn, Long> {
    ...
    /**
     * SELECT … FOR UPDATE: serializa el acceso al turno pendiente.
     * Garantiza que dos llamadas concurrentes a submitMove lean el mismo turno
     * y que la resolución solo ocurra una vez.
     */
    @Query("SELECT t FROM MatchTurn t WHERE t.round = :round AND t.result = :result ORDER BY t.turnNumber ASC")
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    List<MatchTurn> findPendingByRoundForUpdate(@Param("round") MatchRound round,
                                                 @Param("result") TurnResult result);
    ...
    @Query("SELECT t FROM MatchTurn t WHERE t.round.match = :match " +
           "ORDER BY t.round.roundNumber ASC, t.turnNumber ASC")
    List<MatchTurn> findAllByMatchOrdered(@Param("match") Match match);

    @Query("SELECT t FROM MatchTurn t " +
           "WHERE t.round.match.status = 'IN_PROGRESS' AND t.result = 'PENDING'")
    List<MatchTurn> findAllPendingInProgressTurns();

    @Modifying
    @Query("DELETE FROM MatchTurn t " +
           "WHERE t.round.match.player1.id = :personId OR t.round.match.player2.id = :personId")
